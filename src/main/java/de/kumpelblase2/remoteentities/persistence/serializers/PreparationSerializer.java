...
public abstract class PreparationSerializer implements IEntitySerializer {
    ...
    public RemoteEntity create(EntityData inData) {
        EntityManager manager = RemoteEntities.getManagerOfPlugin(this.m_plugin
                .getName());
        if (inData.type == null) {
            inData.type = RemoteEntityType.Human;
            System.out.println("#### hotfix");
        }
        CreateEntityContext context = manager.prepareEntity(inData.type);
        context.withName(inData.name).asPushable(inData.pushable)
                .asStationary(inData.stationary).withID(inData.id);
        context.withSpeed(inData.speed).withPathfindingRange(
                inData.pathfindingRange);
        ...
        for (DesireData data : inData.movementDesires) {
                ...
                entity.getMind().addMovementDesire(item.getDesire(),
                        item.getPriority());
        }

        for (DesireData data : inData.actionDesires) {
            DesireItem item = data.create(entity);
            entity.getMind().addTargetingDesire(item.getDesire(),
                    item.getPriority());
        }
